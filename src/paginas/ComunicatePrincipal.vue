  ...
  <div class="container mx-auto px-6 mb-12">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-4xl mx-auto">
      ...
      <div>
        ...
        <a href="https://goo.gl/maps/3CWVHKY4pBS96pSx9?coh=178572&entry=tt" target="_blank"
      ...
      <div>
        <div name="contact" netlify>
          <div>
            ...
            <input v-model="mensajes.name" name="Nombre" type="text" placeholder="Nombre" required
          ...
          <div class="mt-6">
            ...
            <input v-model="mensajes.email" name="email" type="email" placeholder="tecuidamosunicor@email.com" required
              class="mt-2 w-full border-none text-sm p-4 bg-gray-100 text-gray-700" />
          </div>
          <div class="mt-6">
            <label class="block text-base tracking-tight text-gray-600">Mensaje</label>
            <textarea v-model="mensajes.mensaje" name="message" placeholder="Escriba su mensaje" required
              class="mt-2 w-full border-none text-sm p-4 bg-gray-100 text-gray-700"></textarea>
          </div>
          <p class="mb-10" v-if="user == 0">Inicie sesion para enviar un mensaje <span
              class="text-red-700 font-extrabold text-xl">*</span></p>
          <button v-if="user == 1" @click="enviarMensaje"
            class="inline-block bg-lime-800 text-white uppercase text-sm tracking-widest font-heading px-8 py-4 mt-6">
            Enviar mensaje
          </button>
        </div>
      </div>
...
export default {
  beforeMount() {
    if (localStorage.getItem("user") != 'null') {
      ...
      this.mensajes.name = usuario.name
      this.mensajes.email = usuario.email
      this.mensajes.id_user = usuario.id_user
    }
  ...
  methods: {
    enviarMensaje() {
      axios({
        method: 'post',
        url: 'https://apigenerator.dronahq.com/api/lzNnYU-E/mensajes',
        data: this.mensajes,
        responseType: 'json',
      })
        .then(response => {
